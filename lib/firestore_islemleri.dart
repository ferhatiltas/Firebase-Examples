...
class _FirestoreIslemleriState extends State<FirestoreIslemleri> {
      ...
      body: Center(
        child: Column(mainAxisAlignment: MainAxisAlignment.start,
          children: [
            ...
            Expanded(child: _secilenResim == null ? Text(
              "Resim Yok", style: TextStyle(fontSize: 22),) : Image.file(
                _secilenResim as File))
          ],
        ),
      ),
    );
  ...
  void _veriSorgula() async {
    ...
    var dokumanlar = await _firestore.collection("users").where(
        "isim", isEqualTo: 'Bilinmeyen').get();
  ...
  void _galeriResimUpload() async {
    ...
    var resim = await _picker.getImage(source: ImageSource
        .gallery); // galeriden seçilen resmin ekranda gözükmesiniz sağlar
    setState(() {
      _secilenResim = File(
          resim.path); // galeriden seçilen resmin ekranda gözükmesiniz sağlar
    });
    ...
    UploadTask uploadTask = ref.putFile(
        File(_secilenResim.path)); // resmi veritabanında üstteki yola at
    var url = await (await uploadTask).ref
        .getDownloadURL(); // yüklendikten sonra url olutur
